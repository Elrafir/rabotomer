...
<script>
    function searchTasks() {
        ...
        $.post('<?= site_url("calculations/search_tasks_ajax") ?>', { start_date: start, end_date: end }, function(response) {
            try {
                var res = JSON.parse(response);
                if (res.status === 'success') {
                    // Update inner html
                    container.html(res.html);
                    // Add drag events to newly loaded items
    ...
    function addTaskToPackage(taskId) {
        ...
        $.post('<?= site_url("calculations/add_task_ajax") ?>', { package_id: packageId, task_id: taskId }, function(response) {
            try {
                var res = JSON.parse(response);
                if (res.status === 'success') {
    ...
    function removeTaskFromPackage(taskId) {
        var packageId = $('#currentPackageId').val();
        $.post('<?= site_url("calculations/remove_task_ajax") ?>', { package_id: packageId, task_id: taskId }, function(response) {
            try {
                var res = JSON.parse(response);
                if (res.status === 'success') {
                    window.location.reload();
